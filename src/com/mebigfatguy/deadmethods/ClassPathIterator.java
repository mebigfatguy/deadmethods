...
public abstract class ClassPathIterator implements Iterator<String> {

    Iterator<Resource> frIt;
    Iterator<String> subIt = null;

    public ClassPathIterator(Path classPath) {
    ...
    class DirectoryIterator implements Iterator<String> {
        ...
        private String getNextFile() {
            ...
            while (!paths.isEmpty()) {
                ...
                if (file.exists()) {
                    if (file.isFile()) {
                        if (validPath(file.getName())) {
                            return adjustPath(file.getName());
                        }
                    } else {
                        File[] files = file.listFiles(new FileFilter() {
                            @Override
                            public boolean accept(File f) {
                                return f.isDirectory() || (f.isFile() && validPath(f.getPath()));
                            }
                        });
